...
class KryoConvertersTest {
    ...
    @Test
    fun roundTrip_isByteStable() {
        // Serializing the same bean twice produces identical bytes (format stability guard).
        val bean = SOCKSBean().apply {
            serverAddress = "10.0.0.1"
            serverPort = 1080
            initializeDefaultValues()
        }
        assertArrayEquals(KryoConverters.serialize(bean), KryoConverters.serialize(bean))
    }
